...
def get_from_netstat():
    ...
    for i in range(1, len(lines_before)):
        ...
        result_line.append( line_af[0] )
        result_line.append( str(int(line_af[rx_flag]) - int(line_be[rx_flag])) )
        result_line.append( str(int(line_af[tx_flag]) - int(line_be[tx_flag])) )
...
if __name__ == '__main__':
    ...
    for ifname in host_if :
        print "IF [%s] --> %s " %(ifname, get_from_ip(ifname))
